...
//  { name: 'Clouds', colors: [ '#ECE9E6', '#FFFFFF' ] },
  ...
  { name: 'Shadow Night', colors: [ '#000000', '#53346D' ] },
  ...
  { name: 'Instagram', colors: [ '#517fa4', '#243949' ] },
...
export default class Avatar extends React.Component {
  ...
  shouldComponentUpdate(nextProps,nextState) {
    const { props } = this;
    const diff = (
      props.user.user_photo_url != nextProps.user.user_photo_url
  ...
  render() {
    const { user, src } = this.props;

    let inner = null;
    if (src) {
      inner = <CoverImage src={src} />;
    } else if (user && user.user_photo_url) {
      const style = {
        backgroundImage: "url(" + user.user_photo_url + ")",
      };
      inner = <div style={style} />;
    } else {
      const hash = util.stringHash(user.email);
      const index = Math.abs(hash) % AVATAR_COLOR_LIST.length;
      const color_stops = AVATAR_COLOR_LIST[index].colors.join(",");
      const style = {
        backgroundImage: "linear-gradient(to bottom right," + color_stops + ")",
      };
      inner = <div className='avatar-placeholder' style={style} />;
    }
    return (
